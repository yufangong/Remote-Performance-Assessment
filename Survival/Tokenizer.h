...
/*
  ...
  - Double quoted and single quoted strings are returned as single tokens.
    ...
    Toker::returnSingleQuotedStringsAsTokens(false).  That is needed if 
  ...
  ====================
  ver 2.2 : 23 Feb 13
  - fixed bug reported with suggested fix by Kevin Wang.  Use of 
    cctype functions without locale caused crashes in several low-level
    functions that tested character types. Crashes occur in 
    std::cctype.
  ver 2.1 : 28 Jan 13
  - fixed bug reported by Sam Savage in Toker::attach.
    re-initialized prevprevChar, prevChar, currChar, nextChar, 
    numLines, and braceCount
  ver 2.0 : 09 Jun 11
  - deleted pIn in destructor
  ver 1.9 : 20 Feb 11
  - TA Mehmet Kaya fixed bug handling many contiguous escape chars,
    reported by Ider (Jian) Zhang and Harshal Bhakta
  ver 1.8 : 07 Feb 10
  - added returnSingleQuotedStringAsToken(bool doReturn) to support
    XML parsing where text body has single quotes.  Note that if you
    set as false, then single quoted attribute values won't be returned
    as a single token.
  ver 1.7 : 31 Jan 09
...
public:
  ...
  std::string getTok();
  void returnComments(bool doReturn = false);
  void returnSingleQuotedStringAsToken(bool doCollect=true);
  int& lines();
  int  braceLevel();
  bool isFileEnd();
...
private:
  ...
  bool doReturnSingleQuotesAsToken;
  bool aCppComment;
  enum state _state;
  mode _mode;
  bool EndQuoteCounter;
