...
//----< set mode for collecting single quoted strings >--------

void Toker::returnSingleQuotedStringAsToken(bool doReturn) 
{ 
  doReturnSingleQuotesAsToken = doReturn; 
  scTok += "\'";
}
...
void Toker::stripWhiteSpace()
{
  if(nextChar == '\n')
    return;
  std::locale loc;
  while(isspace(nextChar, loc) && nextChar != '\n')
  {
    getChar();
  }
}
...
{
  std::locale loc;
  if(isalpha(ch, loc) || ch == '_' || isdigit(ch, loc))
    return true;
  return false;
}
...
{
  if(prevChar != '\\' && currChar == '\'' && doReturnSingleQuotesAsToken)
  {
    aSingleQuote = true;
    return true;
  }
...
bool Toker::isEndQuote()
{
  ...
  {
    if(prevChar=='\\')
      EndQuoteCounter = !EndQuoteCounter;
    else
      EndQuoteCounter = false;
  }
  else
    EndQuoteCounter = true;

  if(prevChar == '\\' || currChar != '\\')
  {
    if(aSingleQuote && nextChar == '\'' && EndQuoteCounter)
    {
      EndQuoteCounter = false;
      return true;
    }
